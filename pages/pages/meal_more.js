...
function Meal_more() {
    ...
    const url = `https://open.neis.go.kr/hub/mealServiceDietInfo?KEY=${key}&TYPE=json&pIndex=1&pSize=20&SD_SCHUL_CODE=${school_ID_data}&ATPT_OFCDC_SC_CODE=${school_REGION_data}&MLSV_FROM_YMD=${YYYYMMDD}`
    ...
    useEffect(() => {
        (async () => {
            const {mealServiceDietInfo} = await (await fetch(url)).json();
            console.log(mealServiceDietInfo)
        })();
    }, [])
    return (
        <View style={styles.main}>
            <ScrollView showsVerticalScrollIndicator={false} showsHorizontalScrollIndicator={false} style={styles.content}>{meal_data}</ScrollView>
        </View>
    )
}
